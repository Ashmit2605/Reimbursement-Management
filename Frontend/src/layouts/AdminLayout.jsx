    ...
    <div className="relative flex min-h-screen bg-gray-50 font-['Poppins',sans-serif]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 border-b border-gray-100 flex items-center justify-between">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 text-[#388087] hover:bg-gray-50 rounded-lg">
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              <NavLink key={item.path} to={item.path} className={({ isActive }) => `w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 ${isActive ? 'bg-gradient-to-r from-[#388087]/10 to-[#6FB3B8]/10 text-[#388087] font-medium' : 'text-[#388087]/70 hover:bg-gray-50 hover:text-[#388087]'}`}>
                <Icon size={20} strokeWidth={2} />
                <span className="flex-1 text-left text-[15px]">{item.label}</span>
                {item.badge && <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-600">{item.badge}</span>}
              </NavLink>
            )
          })}
        </nav>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 text-[#388087] hover:bg-gray-100 rounded-lg">
            <Menu size={24} />
          </button>
